...
接下来的事情，交由脚本来做了，静静的等待抢票结果就好（刷票过程中，浏览器不可关闭）
author: xiangxl
time: 2023-07-06
"""
...
class BrushTicket(object):
    ...
    def __init__(self, user_name, password, passengers, from_time, from_station, to_station, number, seat_type, receiver_mobile, receiver_email):
        ...
        self.executable_path = 'D:\Program Files\python3.9\Scripts\chromedriver.exe'
 
    def do_login(self):
        """登录功能实现，手动识别验证码进行登录"""
        self.driver.visit(self.login_url)
        sleep(1)
        self.driver.find_by_id('J-userName')[0].fill(self.user_name)
        self.driver.find_by_id('J-password')[0].fill(self.password)
        self.driver.find_by_id('J-login')[0].click()
        while True:
    ...
    def start_brush(self):
        ...
        try:
            ...
            while self.driver.url.split('?')[0] == self.ticket_url:
                ...
                try:
                    car_no_location = self.driver.find_by_id("queryLeftTable")[0].find_by_xpath("//a[contains(text(),'" + self.number + "')]")
    ...
    def send_mail(self, receiver_address, content):
        ...
        host = 'smtp.163.com'
        port = 25
        sender = 'm15732633722@163.com'  # 你的发件邮箱号码
        # 不是登陆密码，是客户端授权密码
        pwd = 'URBIYUSUXSHDHFDF'
        # 发件信息
...
if __name__ == '__main__':
    ...
    L1 = Label(root, text="12306用户名").grid(row=0, column=0)
    E1 = Entry(root, bd = 5)
    E1.grid(row=0, column=1)
    L2 = Label(root, text="12306登录密码").grid(row=1, column=0)
    E2 = Entry(root, bd = 5, show="*")
    E2.grid(row=1, column=1)
    L3 = Label(root, text="乘车人姓名(多人用英文逗号“,”连接)").grid(row=2, column=0)
    E3 = Entry(root, bd = 5)
    E3.grid(row=2, column=1)
    L4 = Label(root, text="乘车日期（例如“2018-08-08”）").grid(row=3, column=0)
    E4 = Entry(root, bd = 5)
    E4.grid(row=3, column=1)
    L5 = Label(root, text="出发站（例如北京“bj”）").grid(row=4, column=0)
    E5 = Entry(root, bd = 5)
    E5.grid(row=4, column=1)
    L6 = Label(root, text="终点站（例如北京“bj”）").grid(row=5, column=0)
    E6 = Entry(root, bd = 5)
    E6.grid(row=5, column=1)
    L7 = Label(root, text="车次号（例如“G110”）").grid(row=6, column=0)
    E7 = Entry(root, bd = 5)
    E7.grid(row=6, column=1)
    L8 = Label(root, text="座位类型（例如“软卧”）").grid(row=7, column=0)
    E8 = Entry(root, bd = 5)
    E8.grid(row=7, column=1)
    L9 = Label(root, text="通知手机号").grid(row=8, column=0)
    E9 = Entry(root, bd = 5)
    E9.grid(row=8, column=1)
    L10 = Label(root, text="通知邮箱").grid(row=9, column=0)
    E10 = Entry(root, bd = 5)
    E10.grid(row=9, column=1)
    def getTextInput():
        user_name = E1.get()  # 获取文本输入框的内容
        password = E2.get()
        # 乘客姓名
        passengers = E3.get().split(",")
        # 乘车日期
        from_time = E4.get()
        city_list = {
            'bj': '%u5317%u4EAC%2CBJP',  # 北京
            ...
            'wzn': '%u68A7%u5DDE%u5357%2CWBZ',  # 梧州南
            'hz': '%u676D%u5DDE%2CHZH', #杭州
            'dgd': '%u4E1C%u839E%u4E1C%2CDMQ', #东莞东
            'sh': '%u4E0A%u6D77%2CSHH', #上海
            'tmn': '%u5929%u95E8%u5357%2CTNN', #天门南
        }
        # 出发站
        from_input = E5.get()
        from_station = city_list[from_input]
        ...
        seat_type = E8.get()
        # 抢票成功，通知该手机号码
        receiver_mobile = E9.get()
        # 抢票成功，通知该邮箱
        receiver_email = E10.get()
        root.destroy()
        ticket = BrushTicket(user_name, password, passengers, from_time, from_station, to_station, number, seat_type, receiver_mobile, receiver_email)
        ticket.start_brush()
    Button_login = Button(root,text = "一键抢票",width = 8,font = ("华文黑体",14), command=getTextInput).grid(row = 11,column = 1,sticky = "e")
